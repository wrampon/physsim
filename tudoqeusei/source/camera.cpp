...
void Camera::Reset(int width, int height)
{
    ...
    m_head = 0.0;
    m_pitch = 0;
    m_lookat = glm::vec3(0.0, 0, 0.0);
    m_up = glm::vec3(0.0, 1.0, 0.0);
	m_right = glm::vec3(1.0, 0, 0);
    m_fovy = 60.0;
    m_width = width;
    m_height = height;
    m_znear = 0.01;
...
void Camera::targetPosition(glm::vec3 camposition, glm::vec3 targetPosition)
{	
	m_eye_distance = glm::length(camposition);
	m_lookat = targetPosition;
	updateViewMatrix();

	glm::vec3 dv = glm::normalize(camposition);
	float t_head = glm::degrees(asin(dv.y));
	float t_pitch = glm::degrees(atan2(dv.z, dv.x));
...
{
    m_eye_distance -= dy * coe*0.1;
    if (m_eye_distance < 0.001) 
		m_eye_distance = 0.001; 
    updateViewMatrix();
}
...
void Camera::updateViewMatrix()
{
    ...
    m_position.x = m_lookat.x + m_eye_distance * glm::cos(r_head) * glm::cos(r_pitch); //pitch = phi
    m_position.y = m_lookat.y + m_eye_distance * glm::sin(r_head); //head = head = theta
	...
	// Vector A = C x U
	m_right = glm::normalize(glm::cross(C, U));
