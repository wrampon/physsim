...
void main()
{
    ...
    float totalTerm = (diffuseTerm1 + diffuseTerm2) / 2 * 0.7 + 0.3; //(diffuseTerm1 + diffuseTerm2 + diffuseTerm3) / 2 * 0.7 + 0.3;

    if (u_choose_tex != 0)
		out_Color = texture(u_sampler1, vec2(f_texcoord)) * totalTerm + diffuseColor;
    else
